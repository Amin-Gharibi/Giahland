			...
			<div className={"w-full flex justify-between items-center"}>
				<div className={"flex justify-between items-center gap-x-3 sm:gap-x-6 lg:gap-x-4 xl:gap-x-6"}>
					...
					<div className={"hidden lg:flex items-center lg:gap-x-4 xl:gap-x-8 max-sm:hidden lg:*:text-base xl:*:text-lg"}>
						...
						<Link className={location.pathname === "/herbal-medicine" ? "text-primary" : ""} to={"/herbal-medicine"}>
							گیاه پزشک
						</Link>
				...
				<div className={"flex items-stretch gap-x-2 sm:gap-x-4"}>
					<div className={"max-sm:hidden h-auto"}>
						{showSearchInput ? (
							<div className="h-full flex items-stretch bg-transparent py-2 px-2.5 rounded-lg bg-neutral1 border border-dashed border-primary">
								<input type="text" placeholder="جستجو گیاه" {...searchRegister("searchText")} className="max-md:w-[170px] lg:w-[150px] xl:w-auto bg-transparent border-none outline-none text-sm xl:text-base text-neutral9" />
					...
					<div className={"max-sm:hidden"}>
						<CustomIconButton onClick={() => true} size={48} icon={shoppingCartPrimary} isOutline={true} isSquared={true} />
					</div>
					<div className={"sm:hidden"}>
						<CustomIconButton onClick={() => true} size={32} icon={shoppingCartPrimary} isOutline={true} isSquared={true} />
					</div>
					<div className={"max-sm:hidden"}>
						<CustomButton title={"ورود | ثبت نام"} onClick={() => true} size={48} icon={loginPrimary} leftIcon={true} isOutline={true} isSquared={true} />
					</div>
					<div className={"sm:hidden"}>
						<CustomIconButton onClick={() => true} size={32} icon={login2Primary} isOutline={true} isSquared={true} />
					</div>
				</div>
			</div>
			<form onSubmit={searchHandleSubmit(handleSearching)} className={"sm:hidden mt-4"}>
				<div className="flex items-stretch h-10 bg-neutral3 py-2 px-2.5 rounded-lg">
					<input type="text" placeholder="جستجو گیاه" {...searchRegister("searchText")} className="flex-grow bg-transparent border-none outline-none text-sm text-neutral9" />
					<button className={"outline-none flex items-center justify-center p-1"}>
						<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
